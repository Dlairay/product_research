...
def gpt_sentiment_analysis(text, model="gpt-4o"):
    ...
    # print(len(column))
    prompt = f"""Analyze the sentiment of comment and return ONLY numerical output :
...
def gpt_feedback_classifier(text, model="gpt-4o"):
    prompt = f"""Classify the comment below as either True or False based on whether it contains product feedback or suggestions.
    - Return ONLY the exact word True if it includes feedback/suggestions about a product.
    - Return ONLY the exact word False otherwise.
...
def run_all(df, n_samples=50, model="gpt-3.5-turbo", seed=42):
    ...
    df["is_feedback"] = df["content"].apply(gpt_feedback_classifier)
    # df["sentiment_score"] = df.apply(
    #     lambda row: analyze_sentiment_score(row["content"]) if row["is_feedback"] else None,
    #     axis=1
    # )
    df["sentiment_score"] = df["content"].apply(gpt_sentiment_analysis) 
